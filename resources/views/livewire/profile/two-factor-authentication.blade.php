<div>
    <section class="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
        ...
        @if ($totpEnabled)
            ...
            <div class="mt-4 rounded-md bg-green-50 p-4 dark:bg-green-900/20">
                <p class="text-sm font-medium text-green-700 dark:text-green-400">
                    {{ __('profile.twofa_totp_enabled_status') }}
                </p>
            </div>
        ...
        @elseif ($emailEnabled)
            ...
            <div class="mt-4 rounded-md bg-green-50 p-4 dark:bg-green-900/20">
                <p class="text-sm font-medium text-green-700 dark:text-green-400">
                    {{ __('profile.twofa_email_enabled_status') }}
                </p>
            </div>
            ...
            <div class="mt-6">
                <button
                    type="button"
                    wire:click="disableEmailTwoFactor"
                    class="rounded-md bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
                >
                    {{ __('profile.twofa_disable') }}
                </button>
        ...
        @elseif ($enablingEmail)
            {{-- Email 2FA confirmation step --}}
            <div class="mt-4 space-y-4">
                <p class="text-sm text-gray-600 dark:text-gray-400">
                    {{ __('profile.twofa_email_confirm_description') }}
                </p>

                <form wire:submit="confirmEmailTwoFactor">
                    <div>
                        <label for="email_code" class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                            {{ __('profile.twofa_email_confirm_code') }}
                        </label>
                        <input
                            type="text"
                            id="email_code"
                    ...
                    <div class="mt-4 flex items-center gap-4">
                        ...
                        >
                            {{ __('profile.twofa_email_confirm') }}
                        </button>
        ...
        @else
            {{-- 2FA is not enabled — show method choices --}}
            <div class="mt-4 space-y-6">
                {{-- TOTP option --}}
                <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                    <h3 class="text-sm font-medium text-gray-900 dark:text-gray-100">
                        {{ __('profile.twofa_method_totp') }}
                    </h3>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        {{ __('profile.twofa_scan_instructions') }}
                    </p>

                    <div class="mt-3" x-data="{ enabling: false, confirming: false, qrCode: '', confirmationCode: '' }">
                ...
                <div class="rounded-lg border border-gray-200 p-4 dark:border-gray-700">
                    <h3 class="text-sm font-medium text-gray-900 dark:text-gray-100">
                        {{ __('profile.twofa_method_email') }}
                    </h3>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                        {{ __('profile.twofa_email_description') }}
                    </p>

                    <div class="mt-3">
                        <button
                            type="button"
                            wire:click="enableEmailTwoFactor"
                            class="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                        >
                            {{ __('profile.twofa_email_enable') }}
                        </button>
                    </div>
                </div>
            </div>
