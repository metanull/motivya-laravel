<div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div class="mb-4">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">
            {{ __('accountant.anomalies_heading') }}
        </h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {{ __('accountant.anomalies_subtitle') }}
        </p>
    </div>
    ...
    <div class="mb-4">
        <select
            ...
            <option value="">{{ __('accountant.anomalies_filter_all_types') }}</option>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
                <h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">
                    {{ __('accountant.anomalies_confirm_resolve') }}
                </h2>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ __('accountant.anomalies_resolve_reason_label') }}
                </label>
                <textarea
                    ...
                    placeholder="{{ __('accountant.anomalies_resolve_reason_placeholder') }}"></textarea>
                ...
                <div class="mt-4 flex justify-end gap-3">
                    ...
                    <button wire:click="resolve({{ $resolvingAnomalyId }})"
                        ...
                        {{ __('accountant.anomalies_action_resolve') }}
                    </button>
                </div>
            </div>
        </div>
    ...
    @if ($ignoringAnomalyId !== null)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl dark:bg-gray-800">
                <h2 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">
                    {{ __('accountant.anomalies_confirm_ignore') }}
                </h2>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">
                    {{ __('accountant.anomalies_ignore_reason_label') }}
                </label>
                <textarea
                    wire:model="ignoreReason"
                    rows="3"
                    class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-200"
                    placeholder="{{ __('accountant.anomalies_ignore_reason_placeholder') }}"></textarea>
                ...
                <div class="mt-4 flex justify-end gap-3">
                    ...
                    <button wire:click="ignore({{ $ignoringAnomalyId }})"
                        ...
                        {{ __('accountant.anomalies_action_ignore') }}
                    </button>
                </div>
            </div>
        </div>
            ...
            <thead class="bg-gray-50 dark:bg-gray-900">
                <tr>
                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                        {{ __('accountant.anomalies_col_type') }}
                    </th>
                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                        {{ __('accountant.anomalies_col_description') }}
                    </th>
                    <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                        {{ __('accountant.anomalies_col_recommended_action') }}
                    </th>
                    <th class="px-4 py-3 text-right text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-400">
                        {{ __('accountant.anomalies_col_actions') }}
                    </th>
                </tr>
            </thead>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-4 py-8 text-center text-sm text-gray-500 dark:text-gray-400">
                            {{ __('accountant.anomalies_no_results') }}
                        </td>
                    </tr>
                @endforelse
            </tbody>
